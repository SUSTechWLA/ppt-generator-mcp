...
2. `create_template_from_reference` accepts exactly one inline HTML reference, bounded PNG/JPEG/WebP data URL, or strict `TemplateBlueprint`.
   ...
   - If no multimodal analyzer is configured, image input returns `needs_analysis`, a stable prompt, and the public blueprint **JSON Schema + `x-serverValidation` contract**. JSON Schema describes bounded field shapes; the machine-readable role/component mapping and cross-field server rules describe constraints that draft-7 JSON Schema alone cannot express. It does not create a knowledge record or compiled files. A caller can submit the validated blueprint in a later call.
...
3. `list_template_knowledge` returns immutable logical knowledge IDs, source type/hash, template version, capability tags, closed artifact names, and versioned Chromium QA evidence. It never returns physical paths or raw source data.
   - `quality.evidenceVersion: 2` with `imageEvidenceStatus: measured` includes measured raster count/area and zero collision/containment gates.
   - Records written by the original v1 store remain readable as `quality.evidenceVersion: 1` with `imageEvidenceStatus: not-recorded`. They retain their original Chromium hard-gate result but deliberately do not expose or infer raster metrics that were never persisted.
...
The store reads both legacy index version 1 and current index version 2. Legacy request entries are deterministically interpreted as approved only when they contain a valid knowledge ID that resolves to an immutable record; the original separate pending-analysis index is merged into the same fingerprint boundary. Corrupt, ambiguous, or conflicting legacy entries fail closed. New mutations are serialized per server-owned root and written as index version 2.
