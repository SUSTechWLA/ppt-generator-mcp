...
export function evaluateDeterministic(render: RenderResult, policy: DeterministicEvaluationPolicy = {}): DeterministicReport {
  ...
  if (render.signals.noCjkFont) issue({ severity: "error", category: "technical", evidence: "\u9875\u9762\u5305\u542b\u4e2d\u6587\u6587\u672c\u4f46\u7cfb\u7edf\u6ca1\u6709\u53ef\u7528\u7684\u4e2d\u6587\u5b57\u4f53\uff0c\u4ea4\u4ed8\u4ef6\u53ef\u80fd\u6e32\u67d3\u4e3a\u7f3a\u5b57\u65b9\u6846", suggestedAction: "\u5b89\u88c5\u4e00\u79cd\u53ef\u7528\u7684\u4e2d\u6587\u5b57\u4f53\uff08\u5982 Noto Sans CJK\uff09\u540e\u91cd\u65b0\u751f\u6210" }, true);
  ...
  for (const element of render.elements) {
    ...
    if ((clippedHorizontally || clippedVertically) && element.textOverflow !== "ellipsis") issue({ severity: "error", category: "layout", targetId: element.id, evidence: `${element.tag} \u5b58\u5728\u6587\u672c\u6eda\u52a8\u6ea2\u51fa`, suggestedAction: "\u5b9a\u5411\u6539\u5199\u8be5\u6a21\u5757\u6216\u5207\u6362\u6a21\u677f" });
  ...
  for (const [index, image] of render.images.entries()) {
    const imageTargetId = image.assetId ?? `image-${index + 1}`;
    if (!/^data:image\/(?:png|jpeg|webp|svg\+xml);base64,/i.test(image.src)) issue({ severity: "error", category: "asset", targetId: imageTargetId, evidence: "\u56fe\u7247\u4e0d\u662f\u5141\u8bb8\u7684\u5185\u8054\u672c\u5730\u4f4d\u56fe\u6216\u77e2\u91cf\u8d44\u4ea7", suggestedAction: "\u5c06\u5df2\u9a8c\u8bc1\u56fe\u7247\u5185\u8054\u4e3a data URL" }, true);
    else if (!image.complete || image.naturalWidth === 0 || image.naturalHeight === 0 || image.opaqueRatio < 0.02) issue({ severity: "error", category: "asset", targetId: imageTargetId, evidence: "\u56fe\u7247\u672a\u52a0\u8f7d\u6216\u6709\u6548\u50cf\u7d20\u4e0d\u8db3", suggestedAction: "\u91cd\u65b0\u6ce8\u5165\u6709\u6548\u56fe\u7247\u8d44\u4ea7" }, true);
    else if (!image.isVector && image.luminanceVariance < 0.0005) issue({ severity: "warning", category: "asset", targetId: imageTargetId, evidence: "\u56fe\u7247\u89c6\u89c9\u53d8\u5316\u8fc7\u4f4e\uff0c\u53ef\u80fd\u662f\u5360\u4f4d\u8272\u5757", suggestedAction: "\u68c0\u67e5\u6216\u91cd\u65b0\u751f\u6210\u56fe\u7247" });
  }
