...
export function extractCanonicalAnchors(text: string): CanonicalCriticalAnchor[] {
  ...
  const excludedBareCompounds = new Set([
    ...
    "需求", "所需", "供需", "刚需", "军需", "内需", "外需",
  ]);
  const containsSyntacticBareObligation = (clause: string): boolean => {
    for (let index = 0; index < clause.length; index += 1) {
      const marker = clause[index];
      if (marker !== "应" && marker !== "需") continue;
      const leftCompound = index > 0 ? clause.slice(index - 1, index + 1) : "";
      const rightCompound = clause.slice(index, index + 2);
      if (excludedBareCompounds.has(leftCompound) || excludedBareCompounds.has(rightCompound)) continue;
      return true;
    }
    return false;
  };
  ...
  for (let index = 0; index <= text.length; index += 1) {
    ...
    if (end > start && structuralClause.test(clause)) addAnchor("subject", start, end);
    if (end > start && (strongObligationClause.test(clause) || containsSyntacticBareObligation(clause))) {
      addAnchor("obligation", start, end);
    }
    clauseStart = index + 1;
  }
